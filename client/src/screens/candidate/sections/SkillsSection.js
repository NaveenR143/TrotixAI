...
const SkillsSection = ({ userId, profile, initialSkills, onSuccess, enhancedData }) => {
  ...
  const fetchDropdownData = async (search = "") => {
    ...
    try {
      ...
      if (!result.error) {
        setSkillsDropdown(result.data.map(s => toTitleCase(typeof s === 'string' ? s : s.name)));
      }
    } catch (err) {
  ...
  const handleSkillChange = (newSkills) => {
    const titleCasedSkills = newSkills.map(s => toTitleCase(s));
    setSkills(titleCasedSkills);
        ...
        <Stack spacing={2}>
          ...
          <Autocomplete
            ...
            options={(skillsDropdown || [])
              .map((s) => toTitleCase(typeof s === 'string' ? s : s.name))
