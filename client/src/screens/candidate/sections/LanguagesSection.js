...
const LanguagesSection = ({ userId, profile, initialLanguages, onSuccess, enhancedData }) => {
  ...
  // useEffect(() => {
  //   if (enhancedData) {
  //     setLanguages(Array.isArray(enhancedData) ? [...enhancedData] : []);
  //     setIsEditing(true);
  ...
  const fetchDropdownData = async (search = "") => {
    ...
    try {
      ...
      if (!result.error) {
        setLanguagesDropdown(result.data.map(l => toTitleCase(typeof l === 'string' ? l : l.language)));
      }
    } catch (err) {
  ...
  const handleLanguageChange = (newLanguages) => {
    const titleCasedLanguages = newLanguages.map(l => toTitleCase(l));
  ...
  return (
    ...
    >
      ...
      {/* {showReviewBanner && (
        ...
        >
          ✨ <strong>AI Enhanced:</strong> We've suggested languages based on your profile. Please review and <strong>Save</strong> to apply.
        </Alert>
      )} */}
