...
const CareerAdvisorScreen = () => {
    ...
    const { id: userid, mobile: userMobile, points: userPoints } = useSelector((state) => state.UserReducer);
    ...
    const fetchCareerAdvice = async (isRegenerating = false) => {
        ...
        try {
            const userId = profile?.id || userid || localStorage.getItem("user_id");
            const phone = profile?.personalDetails?.phone || userMobile || localStorage.getItem("mobile_number");
            ...
            // Step 2: If no existing advice found or if we are regenerating, check credits and fetch/generate new advice
            if (!result) {
                // Check balance in Redux store first
                ...
                // Deduct credits for generating new advice
                const creditResult = await profileAPI.deductFeatureCredits(userId, "learning_path");
    ...
    useEffect(() => {
        const fetchBalance = async () => {
            const userId = profile?.id || userid || localStorage.getItem("user_id");
            if (userId) {
                const result = await profileAPI.fetchWalletBalance(userId);
    ...
    const isProfileIncomplete = !profile?.skills?.length || !profile?.education?.length;
                ...
                <Container maxWidth="sm">
                    <Paper elevation={0} sx={{ p: { xs: 4, md: 6 }, textAlign: 'center', borderRadius: 6, border: '1px solid #e2e8f0', bgcolor: 'white', boxShadow: '0 20px 25px -5px rgb(0 0 0 / 0.05)' }}>
                        ...
                        <Typography variant="body1" color="text.secondary" sx={{ mt: 2, mb: 4, lineHeight: 1.6, fontSize: '1.05rem' }}>
                            We're crafting your personalized career roadmap.
                        </Typography>
                        <LinearProgress sx={{ height: 6, borderRadius: 3, bgcolor: '#f1f5f9', '& .MuiLinearProgress-bar': { bgcolor: '#0f172a', borderRadius: 3 } }} />
                    </Paper>
                </Container>
            </Box>
        );
                            ...
                            <Box sx={{ position: "relative", zIndex: 1 }}>
                                <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" alignItems={{ xs: 'flex-start', md: 'center' }} spacing={3}>
                                    ...
                                    <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
                                        ...
                                        >
                                            {refreshing ? "Refreshing..." : "Analyze Again (20 Credits)"}
                                        </Button>
                                    </Stack>
                                </Stack>
                            </Box>
                            <PsychologyIcon sx={{ position: "absolute", right: -30, bottom: -30, fontSize: 240, opacity: 0.04, transform: "rotate(-10deg)" }} />
                        </Paper>
                    </Grid>
...
export default CareerAdvisorScreen;