...
class CreditTxTypeEnum(str, enum.Enum):
    ...
    ai_email_write = "ai_email_write"
    ai_content_write = "ai_content_write"
    ai_resume_improve = "ai_resume_improve"
...
class JobseekerProfile(Base):
    ...
    )  # pgvector column — use NullType to avoid varchar cast
    gender = Column(
...
class JobPosting(Base):
    ...
    company_apply = Column(Boolean, default=False, nullable=True)
...
class JobsViewed(Base):
    __tablename__ = "jobs_viewed"
    __table_args__ = (PrimaryKeyConstraint("job_id", "user_id"),)
    ...
    # Relationships
    job = relationship("JobPosting", backref="views")
...
class UserAIRequest(Base):
    __tablename__ = "user_ai_requests"
    __table_args__ = (
        UniqueConstraint("user_id", "created_at", name="uq_user_ai_request_daily"),
    )
    ...
    enhance_resume = Column(Boolean, default=False)
    skill_analysis = Column(Boolean, default=False)
    learning_path = Column(Boolean, default=False)
    created_at = Column(Date, nullable=False, server_default=func.now(), index=True)
...
class CreditWallet(Base):
    ...
    # Relationships
    user = relationship("User", backref="credit_wallet")
...
class CreditTransaction(Base):
    ...
    type = Column(
        Enum(CreditTxTypeEnum, name="credit_tx_type", create_type=False),
        nullable=False,
    )
