...
const SkillDevelopmentScreen = () => {
    ...
    const { id: userid, points: userPoints } = useSelector((state) => state.UserReducer);
        ...
        try {
            ...
            if (!isRegenerating) {
                ...
                if (!existingResult.error && existingResult.data && existingResult.data !== "none") {
                    ...
                    if (innerData && ((innerData.skills_analysis && innerData.skills_analysis.length > 0) || innerData.industry)) {
                        result = existingResult;
                    }
                }
            }

            // Step 2: If no existing analysis found or if we are regenerating, check credits and fetch/generate new analysis
            if (!result) {
                ...
                // Deduct credits for generating new analysis
                const creditResult = await profileAPI.deductFeatureCredits(userId, "skill_analysis");
                ...
                } else if (creditResult.data?.balance !== undefined) {
                    dispatch(updateUserProfile({ points: creditResult.data.balance }));
                }

                // Fetch/generate new analysis
                result = await profileAPI.fetchMissingSkills(userId, isRegenerating);
            }
