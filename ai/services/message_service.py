...
class MessageService:
    ...
    def send_resumeid_to_queue(self, user_id: str, resume_id: str) -> str:
        """
        Send a message to the queue with user_id and resume_id.
        ...
        try:
            # Create message payload
            message_payload = {"user_id": user_id, "resume_id": resume_id}
            ...
            print(
                f"Successfully sent message to queue '{self.queue_name}' "
                f"for user_id: {user_id}, resume_id: {resume_id}, Message ID: {response.id}"
            )
            return response.id
