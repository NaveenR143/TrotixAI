...
export const getJobDetailsById = async (jobId) => {
    try {
        const response = await axios.get(
            `${API_BASE_URL}${API_ENDPOINTS.GET_JOB}${jobId}`,
            { headers: getHeaders(), timeout: 10000 }
        );
 ...
 * @param {string|number} jobId - ID of the job to deactivate
 */
export const deactivateJob = async (jobId) => {
    try {
        const response = await axios.post(
            `${API_BASE_URL}${API_ENDPOINTS.DEACTIVATE_JOB}`,
            null,
            {
                params: { job_id: jobId },
                headers: getHeaders(),
        ...
        return {
            error: false,
            data: response.data,
            message: response.data?.message || "Job deactivated successfully",
        };
    } catch (error) {
