...
import { fetchJobMatchingCandidates } from "../../api/jobpostingAPI";
...
const CandidateFeedScreen = () => {
  ...
  const jobTitle = location.state?.jobTitle || "Job Matching";
  ...
  useEffect(() => {
    const loadCandidates = async () => {
      setLoading(true);
      try {
        const response = await fetchJobMatchingCandidates(jobId);
  ...
  if (loading) {
    return (
      <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: 'calc(100vh - 64px)', gap: 3 }}>
        ...
        <Typography sx={{ color: '#64748b', fontWeight: 500 }}>Finding the best matches for your job...</Typography>
      </Box>
    );
  }
    ...
    return (
      <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: 'calc(100vh - 64px)', gap: 2 }}>
        ...
        <Typography sx={{ color: '#64748b', textAlign: 'center', maxWidth: 400 }}>
          {error ? "We encountered an issue fetching matches." : "Try adjusting the job description or skills to find more candidates."}
        </Typography>
        <Button variant="outlined" startIcon={<ArrowBackIcon />} onClick={() => navigate('/posted-jobs')} sx={{ mt: 2 }}>
          Back to Jobs
        </Button>
      </Box>
    );
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 64px)', bgcolor: '#f8fafc' }}>
      ...
      <Box sx={{ p: 2, bgcolor: '#fff', borderBottom: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          <IconButton onClick={() => navigate('/posted-jobs')} size="small"><ArrowBackIcon /></IconButton>
          <Box>
        ...
        >
          ...
          <Box sx={{ mt: 2, textAlign: 'center' }}>
            <Typography variant="caption" sx={{ color: '#94a3b8', fontStyle: 'italic' }}>
              💡 Swipe left or right to browse candidates
            </Typography>
...
export default CandidateFeedScreen;
