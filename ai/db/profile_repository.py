...
class ProfileRepository:
        ...
        try:
            query = (
                ...
                .options(
                    ...
                    joinedload(User.work_experiences).joinedload(
    ...
    async def _build_profile_response(user: User, session: AsyncSession) -> dict:
            ...
            user_data.update(
                {
                    ...
                    "parsed_summary": (
                        user.resumes[0].parsed_summary if user.resumes else None
                    ),
        ...
        user_data["achievements"] = achievements_list
        ...
        user_data["resumes"] = resumes_list
        ...
        try:
            ...
            industry_result = await session.execute(
                industry_query, {"user_id": str(user.id)}
            )
            user_industries = [
                {"id": row[0], "name": row[1]} for row in industry_result.fetchall()
            ]
        except Exception as e:
            print(f"Error fetching user industries: {str(e)}")

        user_data["user_industries"] = user_industries

        # Extract AI usage for today
        try:
            usage_query = text(
                "SELECT 1 FROM user_ai_requests WHERE user_id = :user_id AND created_at = CURRENT_DATE AND enhance_resume = TRUE LIMIT 1"
            )
            usage_result = await session.execute(usage_query, {"user_id": str(user.id)})
            user_data["has_enhanced_today"] = usage_result.fetchone() is not None
        except Exception as e:
            print(f"Error fetching AI usage for today: {str(e)}")
            user_data["has_enhanced_today"] = False
    ...
    async def update_profile_summary(
        ...
        Args:
            user_id: User UUID
            update_data: Dictionary containing fields to update (e.g. summary)
            session: Async database session
        ...
        try:
            ...
            # Fetch and return updated profile
            updated_profile = await ProfileRepository.get_user_profile_by_id(
                user_id, session
            )
            return {
                "status": "success",
            ...
            print(f"Error updating profile summary: {str(e)}")
            await session.rollback()
            raise

    @staticmethod
    async def submit_manual_profile(
        submission_data: dict, session: AsyncSession
    ) -> dict:
        """
        Submit a complete manual profile in one transaction.

        Args:
            submission_data: Dictionary with all profile fields
            session: Async database session
        ...
        try:
            ...
            user.full_name = submission_data["full_name"]
            user.email = submission_data["email"]
            user.phone = submission_data["phone"]

            # 2. Update JobseekerProfile
            # Ensure jobseeker profile exists
            if not user.jobseeker_profile:
                jobseeker_profile = JobseekerProfile(user_id=user_id)
                session.add(jobseeker_profile)
            ...
            profile = user.jobseeker_profile
            profile.current_location = submission_data.get("current_location")
            profile.preferred_locations = (
                submission_data.get("preferred_locations") or []
            )
            ...
            # 3. Handle Work Experience (Replace all)
            await session.execute(
                text("DELETE FROM work_experiences WHERE user_id = :uid"),
                {"uid": user_id},
            )
            ...
            # 4. Handle Education (Replace all)
            await session.execute(
                text("DELETE FROM education WHERE user_id = :uid"), {"uid": user_id}
            )
            for edu_data in submission_data.get("education") or []:
                edu = Education(
                    user_id=user_id,
            ...
            for skill_name in submission_data.get("skills") or []:
                ...
                js_skill = JobseekerSkill(
                    user_id=user_id, skill_id=skill.id, level="intermediate"
                )
                session.add(js_skill)

            # 6. Handle Languages (Replace all)
            await session.execute(
            ...
            print(f"Error submitting manual profile: {str(e)}")
            await session.rollback()
            raise
    ...
    async def get_skills_dropdown(
        ...
        """
        Get available skills for dropdown with optional search filtering
    ...
    async def get_languages_dropdown(
        ...
        """
        Get available languages for dropdown with optional search filtering

        Args:
            session: Async database session
            search: Optional search string to filter by language name
            limit: Maximum number of results to return (default 100)
        ...
        try:
            ...
            if search:
                query = query.where(Language.language.ilike(f"%{search}%"))

            query = query.order_by(Language.language).limit(limit)
            result = await session.execute(query)
    ...
    async def update_achievement(
        ...
        Args:
            ...
            session: Async database session
            achievement_id: ID of existing achievement (for updates)
        ...
        try:
            if achievement_id:
                ...
                query = select(Achievement).where(
                    Achievement.id == achievement_id, Achievement.user_id == user_id
                )
                result = await session.execute(query)
                ...
                for field, value in achievement_data.items():
                    if field != "id" and hasattr(achievement, field):
                        setattr(achievement, field, value)
            else:
                # Create new achievement
                achievement = Achievement(
                    user_id=user_id,
                    **{k: v for k, v in achievement_data.items() if k != "id"},
                )
    ...
    async def delete_achievement(
        user_id: UUID, achievement_id: int, session: AsyncSession
    ) -> dict:
        """
        Delete achievement
        ...
        Args:
            user_id: User UUID
            achievement_id: ID of achievement to delete
            session: Async database session
