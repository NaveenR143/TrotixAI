...
class CareerAdvisorService:
    ...
    def validate_profile_for_advice(profile_data: Dict[str, Any]) -> List[str]:
        ...
        # Check for industries (multiple or fallback to single)
        user_industries = profile_data.get("user_industries") or []
        if not user_industries and not profile_data.get("industry_id"):
            errors.append("Industry missing")
    ...
    async def generate_career_advice(
        ...
        try:
            # ✅ Step 0: Check Daily AI Request Limit
            from ai.db.resume_repository import ResumeRepository
            resume_repo = ResumeRepository(session)

            # Check for force_refresh case - only then we care about the daily limit
            # if force_refresh:
            #     has_requested_today = await resume_repo.check_daily_ai_limit(str(user_id), "learning_path")
            #     if has_requested_today:
            #         logger.warning(
            #             f"Daily learning_path limit reached for user: {user_id}")
            #         # We could raise 429 here, but usually services return data.
            #         # Let's let the API layer handle the exception if needed,
            #         # but here we'll just return cached data if limit reached.
            #         cached_data = await CareerAdvisorRepository.get_user_advice(user_id, session)
            #         if cached_data:
            #             return cached_data
            #         # If no cache and limit hit (shouldn't happen if they already requested),
            ...
            #     if cached_data:
            #         logger.info(
            #             f"Returning cached career advice for user: {user_id}")
            #         return cached_data
            ...
            # Add industries to structured data
            structured_data["industries"] = gpt_input.get("industries", [])
            ...
            # ✅ Step 7: Log AI usage
            await resume_repo.log_ai_usage(str(user_id), "learning_path")
            ...
            logger.error(
                f"Error in career advice generation: {str(e)}", exc_info=True)
            raise
    ...
    def _build_gpt_input(profile_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        user_industries = profile_data.get("user_industries") or []
        industry_names = [ind.get("name")
                          for ind in user_industries if ind.get("name")]

        # Fallback to industry_name if user_industries is empty
        if not industry_names and profile_data.get("industry_name"):
            industry_names = [profile_data.get("industry_name")]
        ...
        for item in ai_response.get("action_plan") or []:
            if isinstance(item, dict):
                action_plan.append(
                    {
                        "phase": str(
                            item.get("phase") or item.get(
                                "step_number") or "Next Step"
                        ),
                        "action": str(
    ...
    async def generate_skill_development_plan(
        ...
        try:
            ...
            # 2. Get industries (now supports multiple industries)
            user_industries = profile_data.get("user_industries") or []
            industry_ids = [ind.get("id")
                            for ind in user_industries if ind.get("id")]

            # Fallback to single industry_id if user_industries is empty (backward compatibility)
            if not industry_ids and profile_data.get("industry_id"):
                industry_ids = [profile_data.get("industry_id")]

            if not industry_ids:
                logger.warning(
                    f"No industry_ids found for user {user_id}. Market trend analysis may be limited."
                )

            # 3. Fetch skills from job postings in the associated industries that the user lacks
            market_skills = (
                await CareerAdvisorRepository.get_market_trend_skills_by_industries(
                    user_id=user_id, industry_ids=industry_ids, session=session
                )
            )
            ...
            industry_names = [
                ind.get("name") for ind in user_industries if ind.get("name")
            ]
            structured_data = {
                "user_id": str(user_id),
                "industry": ", ".join(industry_names)
                or profile_data.get("industry_name")
                or "Your Industry",
            ...
            # 8. Log AI usage
            await resume_repo.log_ai_usage(str(user_id), "skill_analysis")
    ...
    async def get_existing_skill_analysis(
        ...
        """
        data = await CareerAdvisorRepository.get_user_skill_analysis(user_id, session)
        if data and isinstance(data, dict) and not data.get("industry"):
